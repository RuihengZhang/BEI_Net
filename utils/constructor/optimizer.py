...
def make_optim_with_cfg(log_var_a, log_var_b, model: nn.Module, optimizer_cfg: dict):
    lr = optimizer_cfg["lr"]
    ...
    all_params = list(grouped_params) + [log_var_a, log_var_b]
    optimizer = construct_optimizer(
        params=all_params, lr=lr, optimizer_type=optimizer_type, cfg=chosen_optimizer_cfg
    )
    return optimizer
...
def group_params(cfg, lr, model, optimizer_strategy):
    if optimizer_strategy == "yolov5":
        ...
        if hasattr(model, "module"): # 如果模型有 "module" 属性，则取出
            model = model.module
        if hasattr(model, "get_grouped_params"): # 如果模型有 "get_grouped_params" 方法，则获取参数分组
        ...
        else: # 如果模型没有 "get_grouped_params" 方法，则将所有参数的学习率设置为 lr 的十分之一
